...
func TestReorderRootMoves_BestMoveEndsUpFirst(t *testing.T) {
	...
	for bestIdx := 0; bestIdx < 10; bestIdx++ {
		ml := buildMoveList(10)
		scores := make([]eval.EvaluationScore, 10)
		for i := range scores {
			scores[i] = eval.EvaluationScore(i) // ascending, so last is best...
		}
...
func TestReorderRootMoves_StableForEqualScores(t *testing.T) {
	ml := buildMoveList(5)
	scores := []eval.EvaluationScore{20, 20, 50, 20, 20}
...
func findBestMove(t *testing.T, fen string, player movegen.Player, depth int) board.Move {
	...
	result := engine.FindBestMove(ctx, b, player, SearchConfig{MaxDepth: depth})
