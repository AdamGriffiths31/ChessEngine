...
func NewEloResultsLogger(rootPath string) *EloResultsLogger {
	return &EloResultsLogger{
		rootPath: rootPath,
		mdFile:   filepath.Join(rootPath, "tools", "results", "elo.md"),
		jsonFile: filepath.Join(rootPath, "tools", "results", "elo_results.jsonl"),
	}
}
...
// FormatResults renders the Elo benchmark result as a human-readable report
// for the caller (cmd/ui layer) to display.
func (rl *EloResultsLogger) FormatResults(result *EloResult) string {
	totalScore := 0.0
	for _, g := range result.Games {
		totalScore += g.Result
	}
	...
	lines := []string{
		...
		strings.Repeat("=", 50),
		fmt.Sprintf("Estimated Elo: %d", int(result.EstimatedElo+0.5)),
		fmt.Sprintf("Score: %s/%d", formatScore(totalScore), len(result.Games)),
		fmt.Sprintf("Duration: %v", result.Duration.Round(time.Second)),
		fmt.Sprintf("Results logged to: %s and %s", rl.mdFile, rl.jsonFile),
	}
