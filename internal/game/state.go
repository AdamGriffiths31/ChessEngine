...
func NewEngine() *Engine {
	...
	return &Engine{
		...
		hashHistory: []uint64{book.GetPolyglotHash().HashPosition(initialBoard)},
	}
}
...
func (e *Engine) updateGameOverState() {
	hash := book.GetPolyglotHash().HashPosition(e.state.Board)
...
func (e *Engine) Reset() {
	...
	e.hashHistory = []uint64{book.GetPolyglotHash().HashPosition(initialBoard)}
}
...
func (e *Engine) LoadFromFEN(fen string) error {
	...
	e.hashHistory = []uint64{book.GetPolyglotHash().HashPosition(newBoard)}
	return nil
}
