...
func (w *Weights) init() {
	...
	w.RookOpenFile = S(10, 10)
	w.RookSemiOpenFile = S(5, 5)
	w.QueenOpenFile = S(5, 5)
	w.QueenSemiOpenFile = S(3, 3)
...
func (w *Weights) initPSQT() {
	...
	for pieceType := data.WP; pieceType <= data.WK; pieceType++ {
		for sq := 0; sq < 64; sq++ {
			...
			w.PSQT[data.White][pieceType][flip(sq)] = val
			w.PSQT[data.Black][pieceType][sq] = val
		}
	}
}
