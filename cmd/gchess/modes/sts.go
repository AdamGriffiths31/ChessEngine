...
func (sm *STSMode) getSTSConfig() (STSConfig, error) {
	config := STSConfig{
		...
		MaxPositions: 10,
		EPDDir:       "testdata",
		ResultsFile:  "tools/results/sts_history.md",
		Verbose:      true,
	}
...
func (sm *STSMode) runSTSBenchmark(config STSConfig, epdFiles []string) (*STSResults, error) {
	...
	jsonl, err := bench.NewSTSResultsWriter(sm.rootPath)
	if err != nil {
		return nil, fmt.Errorf("failed to open JSONL results file: %w", err)
	}
	defer func() {
	...
	for _, epdFile := range epdFiles {
		...
		fileResults := scorer.ScoreSuite(ctx, positions, filepath.Base(epdFile), onResult)
		...
		allResults = append(allResults, fileResults.Results...)
	...
	agg := bench.ComputeSTSAggregates(allResults, results.TotalTime)
	results.CorrectMoves = agg.CorrectMoves
	results.NodesPerSecond = int64(agg.NodesPerSec)
	results.AverageDepth = agg.AvgDepth

	results.GitCommit = bench.GitCommit(sm.rootPath)
	results.JSONLFile = jsonl.Path
	...
	summary := bench.STSSummaryRecord{
		...
		Files:           results.Files,
		FileDescription: epdDescription,
		PositionCount:   results.PositionsTested,
		TotalScore:      results.TotalScore,
		MaxScore:        results.MaxScore,
		ScorePercent:    results.ScorePercent,
		CorrectMoves:    results.CorrectMoves,
		STSRating:       results.STSRating,
		Depth:           results.Config.Depth,
		TimeoutSeconds:  results.Config.Timeout,
		AvgTimeMS:       avgTime.Milliseconds(),
		TotalTimeMS:     results.TotalTime.Milliseconds(),
...
func (sm *STSMode) saveResults(results *STSResults) error {
	...
	if err := os.MkdirAll(filepath.Dir(resultsFile), 0750); err != nil {
