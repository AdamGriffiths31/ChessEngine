...
type EloBenchmarkMode struct {
	...
	runner   *benchmark.EloRunner
	logger   *benchmark.EloResultsLogger
}
...
func NewEloBenchmarkMode() *EloBenchmarkMode {
	...
	return &EloBenchmarkMode{
		...
		runner:   benchmark.NewEloRunner(rootPath),
		logger:   benchmark.NewEloResultsLogger(rootPath),
	}
}
...
func (em *EloBenchmarkMode) Run() error {
	...
	em.logger.DisplayResults(result)
...
// getConfig prompts the user for all Elo benchmark configuration.
func (em *EloBenchmarkMode) getConfig() (benchmark.EloConfig, error) {
	anchorCenter, err := em.prompter.PromptForNumber("Anchor center Elo", 600, 3000)
	if err != nil {
		return benchmark.EloConfig{}, err
	}
